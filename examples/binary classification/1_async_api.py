...
from settings import PROMPT_FORMAT, CSV_FILE, DB_PATH
...
async def main():
    ...
    df = pd.read_csv(CSV_FILE, low_memory=False, nrows=20)  # 只读取前 20 行数据
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("Finished! Show the first result to you:")
    for i, res in enumerate(result):
        print(
            f"  {i + 1}. {{'response': {res['response']}, 'success': {res['success']}, 'error': {res['error']}}}"
        )
        break